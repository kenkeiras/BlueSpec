...
(defmethod initialize-instance :after ((page spec-page)
  ...
  (let ((restructured (flatten (xmls-to-rst (get-content content)))))
    (setf (toctreep page) (every (lambda (X) (or (underlinedp X)
                                                 (= (length X) 0)))
...
(defun underlinedp (text)
  "A text is considered underlined if the last 3 characters are the same."
  (let ((l (length text)))
    (and (> l 6)
         (equalp (char text (- l 1))
                 (char text (- l 2)))
         (equalp (char text (- l 1))
                 (char text (- l 3))))))


(defun xmls-to-rst (xml)
  (cond
    ((stringp xml) xml)
    ((atom xml) NIL)
    ((stringp (first xml))
        ...
        (let* ((text (format NIL "~{~a~}" (flatten (get-text xml))))
          ...
          (format NIL "~%~a~%~a~%~%"
                  ...
                  (make-string (length title)
                               :initial-element (char *UNDERLINES*
                                                      (length index))))))

       ((string= (first xml) "a")
        (let ((text (strip (format NIL "~{~a~}" (flatten
                                                 (mapcar #'xmls-to-rst
                                                         (cddr xml)))))))
          (when (> (length text) 0)
            (if (underlinedp text)
